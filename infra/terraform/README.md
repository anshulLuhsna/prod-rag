...
### Windows Users - Quick SSH Key Check

If you're on Windows and want to use existing SSH keys, run this in PowerShell:

```powershell
# Check if you have SSH keys
if (Test-Path "$env:USERPROFILE\.ssh\id_rsa.pub") {
    Write-Host "✅ Found SSH key at: $env:USERPROFILE\.ssh\id_rsa.pub"
    Write-Host "You can use: use_existing_ssh_key = true"
} else {
    Write-Host "❌ No SSH key found. Terraform will generate one for you."
    Write-Host "Or generate one manually: ssh-keygen -t rsa -b 4096"
}
```

If you don't have SSH keys and want to generate them:
```powershell
# Generate SSH key (if you want to create one manually)
...
### 3. SSH Key Setup (Choose One Option)
...
# If True, you have keys! Continue to terraform.tfvars setup below.
```
...
# Check if you have SSH keys
ls ~/.ssh/id_rsa.pub
# If it exists, you have keys!
```

If you have existing keys, add to `terraform.tfvars`:
```hcl
use_existing_ssh_key = true
# Optionally specify path (auto-detects if not specified):
# existing_ssh_public_key_path = "C:\\Users\\YourUsername\\.ssh\\id_rsa.pub"  # Windows
# existing_ssh_public_key_path = "~/.ssh/id_rsa.pub"  # Linux/Mac
```

#### Option B: Generate New SSH Key (Auto-generated by Terraform)
...
If generating new keys, add to `terraform.tfvars` (or leave default):
```hcl
use_existing_ssh_key = false
```

**Note:** Terraform will automatically generate and save the key to `../.ssh/nifty50_rag_key` if you choose Option B.
...
ssh -i "..\.ssh\nifty50_rag_key" dev@<VM_EXTERNAL_IP>

# If using existing key (replace with your actual path)
ssh -i "$env:USERPROFILE\.ssh\id_rsa" dev@<VM_EXTERNAL_IP>
```
...
ssh -i ~/.ssh/id_rsa dev@<VM_EXTERNAL_IP>
```
...
Host nifty50-rag-dev
    ...
    User dev
    IdentityFile C:\Users\<YourUsername>\OneDrive\Desktop\articles\prod-rag\prod-rag\infra\.ssh\nifty50_rag_key
